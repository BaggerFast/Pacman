...
class FromJsonLoader(BaseFromFileLoader):
    ...
    def __load_player_position(self): self.player_pos = TilePos(*self.json["player_pos"])
    def __load_ghosts_position(self): self.ghosts_pos = [TilePos(*pos) for pos in self.json["ghosts_pos"]]
    def __load_fruit_position(self): self.fruit_pos = TilePos(*self.json["fruit_pos"])
    def __load_energizers_position(self): self.energizers_pos = [TilePos(*pos) for pos in self.json["big_dots_pos"]]
    ...
    def __create_seed_map(self):
        ...
        for rect in self.not_dots_rect:
            for x, y in (TilePos(rect[2], rect[3])+(1, 1)).iterator_to((rect[0], rect[1])):
                self.seed_data[y][x] = False
