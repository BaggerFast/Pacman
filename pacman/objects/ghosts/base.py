...
from misc.constants import DISABLE_GHOSTS_COLLISION, DISABLE_GHOSTS_MOVING
...
class Base(Character):
    ...
    def __init__(self, game, start_pos: Tuple[int, int], frightened_time, chase_time, scatter_time) -> None:
        ...
        # Обычные Анимация
        PathManager.get_list(f'{PathManager.IMAGE}/ghost/{type(self).__name__.lower()}', ext='png')
        self.left_walk_anim = Animator(
            PathManager.get_list(f'{PathManager.IMAGE}/ghost/{type(self).__name__.lower()}/left', ext='png'),
            is_rotation=False
        )
        self.right_walk_anim = Animator(
            PathManager.get_list(f'{PathManager.IMAGE}/ghost/{type(self).__name__.lower()}/right', ext='png'),
            is_rotation=False
        )
        self.top_walk_anim = Animator(
            PathManager.get_list(f'{PathManager.IMAGE}/ghost/{type(self).__name__.lower()}/top', ext='png'),
            is_rotation=False
        )
        self.bottom_walk_anim = Animator(
            PathManager.get_list(f'{PathManager.IMAGE}/ghost/{type(self).__name__.lower()}/bottom', ext='png'),
            is_rotation=False
        )
        ...
        self.frightened_walk_anim1 = Animator(
            PathManager.get_list(f'{PathManager.IMAGE}/ghost/fear1', ext='png'),
            is_rotation=False, aura=PathManager.get_image('ghost/aura_blue.png')
        )
        self.frightened_walk_anim2 = Animator(
            PathManager.get_list(f'{PathManager.IMAGE}/ghost/fear2', ext='png'),
            is_rotation=False, aura=PathManager.get_image('ghost/aura_white.png')
        )

        # Анимации съедения
        self.eaten_left_walk_anim = Animator(
            PathManager.get_list(f'{PathManager.IMAGE}/ghost/eaten/left', ext='png'),
            is_rotation=False
        )
        self.eaten_right_walk_anim = Animator(
            PathManager.get_list(f'{PathManager.IMAGE}/ghost/eaten/right', ext='png'),
            is_rotation=False
        )
        self.eaten_top_walk_anim = Animator(
            PathManager.get_list(f'{PathManager.IMAGE}/ghost/eaten/top', ext='png'),
            is_rotation=False
        )
        self.eaten_bottom_walk_anim = Animator(
            PathManager.get_list(f'{PathManager.IMAGE}/ghost/eaten/bottom', ext='png'),
            is_rotation=False
        )
        ...
        super().__init__(
            ...
            self.top_walk_anim,
            start_pos,
            PathManager.get_image(f'ghost/{type(self).__name__.lower()}/aura.png'))
        ...
        self.invisible_anim = Animator(
            PathManager.get_list(f'{PathManager.IMAGE}/ghost/invisible', ext='png'),
            is_rotation=False
        )
        self.love_cell = (0, 0)
