...
class SkinScene(BaseScene):
    ...
    def __get_btn_manager(self):
        buttons = []
        presets = [
            MenuPreset('DEFAULT', lambda: 1),
            MenuPreset('EDGE', lambda: 1),
            MenuPreset('POKEBALL', lambda: 1),
            MenuPreset('GOOGLE', lambda: 1),
            MenuPreset('HALF-LIFE', lambda: 1),
        ]
        buttons.append(
            Button(
    ...
    #
    #     self.__button_controller = ButtonManager(self.game, buttons)
