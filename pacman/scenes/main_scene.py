...
from misc import LevelLoader, Health, PathManager
...
class MainScene(BaseScene):
    ...
    def __prepare_lives_meter(self) -> None:
        ...
        for i in range(int(self.hp)):
            PathManager.get_image(f'pacman/{self.game.skins.current.name}/walk/1.png')
    ...
    def __create_ghost(self):
        difficult = SettingStorage().difficulty
        if difficult == 0:
    ...
    def process_logic(self) -> None:
        if not self.game.sounds.intro.is_busy():
            ...
            if self.pacman.dead_anim.anim_finished and int(self.hp) < 1 \
                and not self.game.sounds.pacman.is_busy():
                self.template = self.screen.copy()
                self.game.timer = pg.time.get_ticks() / 1000
